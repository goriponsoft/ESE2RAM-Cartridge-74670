...
似非RAMディスクについての詳細は、[似非職人工房の似非RAMDiskのページ](http://www.hat.hi-ho.ne.jp/tujikawa/ese/eseram.html)を参照してください。
...
簡単な方法としては、以下のコード(4000h～5FFFhのアドレス範囲に配置する必要があります)をINITエントリの最初で実行します。
...
省略したり別の初期値を書き込む場合は、初期化コードの置かれているバンクが切り替わらないよう、初期化の順序に注意が必要です。