...
このカートリッジで自作メガROMソフトウェアを動作させる場合は、ROMのINITエントリの本体(ジャンプ先アドレス)を4000h～5FFFhの範囲に置き、できるだけ早い段階で6000h～67FFhのセグメントレジスタに0を書き込んでください。書き込んでいないセグメントレジスタの値は不定になりますので、必要に応じて初期化してください。